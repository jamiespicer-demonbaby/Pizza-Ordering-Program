...
# function that prints the menu and instructions, can be called by entering yes at the start of ordering each pizza
...
# function which can be called to show possible toppings and their cost.
...
Please select the topping you want by typing in the name of the topping:
1. Pineapple (+$1.00)
2. Ham (+$1.00)
3. Bacon (+$1.00)
4. Cheese (+$1.00)
...
def not_blank(question):
    # this function checks if the input is blank and does not continue until it has been filled
    while True:
...
def num_check(question):
    # checks if users input is an integer, tells user to put one in if it is not an integer.
    while True:
...
def string_checker(question, num_letters, valid_responses):
    # checks if input matches the valid answers in a pre-made list, tells user if input is invalid.
...
def clean(x):
    # cleans up look of the printed lists by removing ugly useless brackets
...
def calc_pizza_price(var_pizza):
    # calculates the cost of a pizza based on the users choice and returns value
    price = 0
    if var_pizza == "cheese":
        price = 7.5
...
raffle = [1, 2, 3, 4, 5]
delivery_choice = ["delivery", "pickup"]
...
while True:
    # this predetermined variable starts the while loop allowing the program to start
    another = "yes"
    # brief introduction with light instructions and early quit function
    print("\U0001f355 Hello, welcome to the pizza ordering program! \U0001f355 ")
    print()
    print("***** After typing your inputs in this program, press enter key to continue! *****")
    print()
    start_order = string_checker("Would you like to start your pizza order? (y/n): ", 1, yes_no_list)
    if start_order == "no":
        print("Okay, program ending, 0 pizzas were purchased! ")
        quit()

    # users details are put in and validated using the not_blank and num_check function
    print()
    name = not_blank("Please enter your name so we know who the order is for: ")
    phone = num_check("Please enter your phone number in case we need to call you (integer input please): ")
    while another == "yes":
        # user determines the want_instructions variable allowing user to decide if they want to use the print menu
        # function
    ...
    # here is the code that determines the currently being ordered pizzas size.
        if size_choice == "small":
            ...
            size = 0
    # implicate the size choice to the price and use the which topping as none to start the while loop.
        # Temp toppings list will store the current pizzas potential toppings for later appending to the main list.
        pizza_cost = pizza_cost + size
        which_topping = 'none'
        temp_toppings = []

        want_toppings = string_checker("Would you like any toppings on your pizza? (y/n): ",
                                       1, yes_no_list)
    # loop which allows user to get as many toppings as desired, users are able to exit the look if they want to by
        # using 'xxx'
        if want_toppings == "yes":
            show_toppings()
            while which_topping != 'xxx':
                which_topping = string_checker("Type in the extra topping do you want on your pizza? "
                                               "(type in \'xxx\' to stop ordering toppings): ",
                                               1, topping_list)
    ...
    # order summary of most recent pizza
        ...
        print()
        print("*******************************************************************************")
        print("You, {}, ordered a {} {} pizza with the toppings of {} for a final cost ${:.2f}".format(name,
                                                                                                       size_choice,
                                                                                                       which_pizza,
                                                                                                       temp_toppings,
                                                                                                       pizza_cost).
    ...
    # adding the order details to their assigned lists.
        confirm_ord = string_checker("Would you like to confirm the order of this pizza? (y/n): ", 1, yes_no_list)
        if confirm_ord == "yes":
        ...
        print()
        another = string_checker("Would you like to order another pizza? (y/n): ",
                                 1, yes_no_list)
    # allows the user to decide if they want to continue on with the order, ect. order another pizza
        if another == "no":
            break
    if (len(prices_list)) > 0:
        ...
        print("***** Order Summary *****")
        print("Current Order:               ")
        # I created this looping list function to print out the orders all put the details of the order
        for i, j, z in zip(pizza_sold_list, topping_picked, prices_list):
            print(str(i).replace('[', '').replace(']', '').replace("'", ''), "pizza with the toppings of", str(j)
                  .replace("'", '').replace('[', '(').replace(']', ')'), "with a cost of $", z)
        # this code below allows the user to choose their payment from either cash or credit and uses string checker to
        # validify
        print()
        ...
        print()
        # Here is the code which gives users a 20% chance of winning the raffle. It works by having a random integer
        # selected
        ...
        print("Thank you for ordering from us {}, hope to see you soon! ".format(name))
        print()
        print("****** Here is your receipt ******")
        pizza_order_frame = pandas.DataFrame(pizza_order_dict)
        # cleaning of output for the pandas data table using the clean function, and currency function but first
        # converting to
        # string if required
        add_dollars = ['Price']
        ...
        for var_item in add_extra:
            pizza_order_frame[var_item] = pizza_order_frame[var_item].apply(stringer)
        ...
        print()
        # delivery choice
        delivery_choice = string_checker("Pickup or home delivery(+$5), {}?: ".format(name),
                                         1, delivery_choice)
        print()
        ...
        if which_pay == "cash":
            ...
            print("****** Payed with cash. Your final cost {}: ${:.2f} ******".format(name, final_cost))
        elif which_pay == "credit":
            final_cost = sum(prices_list) * 1.05 + extra_cost
            print("****** Payed with credit. Your final cost {}: ${:.2f} ******".format(name, final_cost))
        # program ends
    else:
        print("You ordered 0 pizzas in total")
        print("***** Come back another day! *****")

    reset_order = string_checker("Reset program for a new order (y/n): ".format(name),
                                 1, yes_no_list)
    if reset_order == "yes":
        print("Restarting...")
