...
while another == "yes":
    # user determines the want_instructions variable allowing user to decide if they want to use the print menu function
...
# here is the code that determines the currently being ordered pizzas size.
    if size_choice == "small":
        ...
        size = 0
# implicate the size choice to the price and use the which topping as none to start the while loop. Temp toppings list
# will store the current pizzas potential toppings for later appending to the main list.
    ...
    want_toppings = string_checker("Would you like any toppings on your pizza? (y/n): ",
                                   1, yes_no_list)
# loop which allows user to get as many toppings as desired, users are able to exit the look if they want to by using
# 'xxx'
    if want_toppings == "yes":
        show_toppings()
        while which_topping != 'xxx':
            which_topping = string_checker("Which extra topping do you want on your pizza? (type in \'xxx\' to stop): ",
                                           1, topping_list)
...
# order summary of most recent pizza
    ...
    print()
    print("*******************************************************************************")
    print("You, {}, ordered a {} {} pizza with the toppings of {} for a final cost ${:.2f}".format(name, size_choice,
                                                                                                   which_pizza,
                                                                                                   temp_toppings,
                                                                                                   pizza_cost).replace(
                                                                                                   '[', '(').replace
...
# this code below allows the user to choose their payment from either cash or credit and uses string checker to validify
print()
...
print()
# Here is the code which gives users a 20% chance of winning the raffle. It works by having a random integer selected
...
# cleaning of output for the pandas data table using the clean function, and currency function but first converting to
# string if required
add_dollars = ['Price']
...
if which_pay == "cash":
    print("****** Payed with cash. Your final cost {}: ${:.2f} ******".format(name, sum(prices_list)))
elif which_pay == "credit":
    final_cost = sum(prices_list) * 1.05
